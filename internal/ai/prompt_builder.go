...
Local vs remote — VERY IMPORTANT: tools whose names start with ` + "`local_`" + ` (local_bash / local_write / local_edit) run on the USER'S OWN MACHINE — they do NOT touch any remote asset. When the scenario targets a specific server / database / Redis / Kafka / K8s asset (an SSH / Database / Redis / SFTP tab is open for it, the user names the asset, or the request is clearly about that asset), you MUST use that asset's dedicated remote tool: run_command for SSH, exec_sql / exec_redis / exec_mongo / exec_k8s / kafka_* / sftp_*. Never fall back to local_bash even when the command looks identical — running ` + "`df -h`" + ` via local_bash tells the user nothing about the server they asked about. local_* tools are only correct when the user explicitly asks about their local machine, or when there is no remote asset in scope.`
}
