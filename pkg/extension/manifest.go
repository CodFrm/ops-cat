...
//   - ID must NOT collide with any BuiltinSnippetCategoryIDs.
...
func (m *Manifest) validateSnippets() error {
	...
	for _, c := range m.Snippets.Categories {
		...
		if _, isBuiltin := BuiltinSnippetCategoryIDs[c.ID]; isBuiltin {
	...
	for _, s := range m.Snippets.Seed {
		...
		_, isBuiltin := BuiltinSnippetCategoryIDs[s.Category]
