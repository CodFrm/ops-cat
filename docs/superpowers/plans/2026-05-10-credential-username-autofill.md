...
## Task 3: 后端 — `app_credential.go` 三个 binding 透传 `username`
...
```bash
git add internal/app/app_credential.go
git commit -m "✨ app: SSH 密钥生成/导入 binding 末尾增加 username 参数"
```
